...
def main(model_name, directory, tokenizer_name, dataset_train, dataset_val, dataset_test, num_classes, class_list=None, use_label_encoder=True, problem_type=None, epochs=10, weights=None, run_num=1):
    ...
    def compute_metrics(eval_pred):
        ...
        return {"accuracy": acc, "precision": prec, "recall": rec, "f1_weighted": f1w}
    ...
    training_args = TrainingArguments(output_dir=directory, 
                                    ...
                                    per_device_train_batch_size= 32 if "deberta" not in tokenizer_name else 16,
                                    per_device_eval_batch_size=32 if "deberta" not in tokenizer_name else 16, 
                                    learning_rate=5e-5,
...
if __name__ == "__main__":
    ...
    for n in range(args.num_runs):
        ...
        main(args.model_name, args.directory_coarse, args.tokenizer, "curriculum_learning_training/datasets/coarse_grained_train.csv", "curriculum_learning_training/datasets/coarse_grained_val.csv", "curriculum_learning_training/datasets/coarse_grained_test.csv", num_classes=4, class_list=["fallacy of relevance", "fallacies of defective induction", "fallacies of presumption", "fallacy of ambiguity"], problem_type="single_label_classification", epochs=10, 
                run_num=n)# weights=[0.60662359, 0.52887324, 2.80223881, 9.62820513])
        main(args.model_name, args.directory_big_bench, args.tokenizer, "curriculum_learning_training/datasets/big_bench_train.csv", "curriculum_learning_training/datasets/big_bench_val.csv", "curriculum_learning_training/datasets/big_bench_test.csv", num_classes=2, class_list=["negative", "positive"], problem_type="single_label_classification", epochs=5,
                run_num=n)
