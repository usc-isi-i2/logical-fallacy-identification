...
def get_graph_similarities_by_sentence_transformers(amr_graphs_with_sentences):
    ...
    for index in similarity_matrix_df.index:
        most_similar_graphs = similarity_matrix_df.loc[index].nlargest(10).index.tolist()
        most_similar_graphs_scores = similarity_matrix_df.loc[index].nlargest(10).values
    ...
    results.to_csv(f"{PATH_TO_MOST_SIMILAR_GRAPHS}{'transformers'}.csv", index = False)
