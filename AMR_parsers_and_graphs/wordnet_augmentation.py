...
if __name__ == "__main__":
        ...
        for node1 in nodes:
            for node2 in nodes:
                ...
                if node1_label != node2_label and (node1, node2) not in edges and (node2, node1) not in edges:
                    ...
                    entailment_relationship = check_if_two_verbs_have_entailment_relations(
                    ...
                    meronym_relationship = check_if_words_have_meronym_relationship(
                        node1_label, node2_label)
                    ...
                    holonym_relationship = check_if_words_have_holonyms_relationship(
                        node1_label, node2_label)
    ...
    print(
        f"Number of edges added based on words being synonym: {np.mean(statistics['syns'])}")
    print(
        f"Number of edges added based on words being antonyms: {np.mean(statistics['ants'])}")
    print(
        f"Number of edges added based on words being entailed: {np.mean(statistics['entails'])}")
    print(
        f"Number of edges added based on words being part_of: {np.mean(statistics['meronyms'])}")
