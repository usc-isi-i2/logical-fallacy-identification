...
if __name__ == "__main__":
    ...
    sentences_with_amr_container = joblib.load(PATH_TO_MASKED_SENTENCES_AMRS_WITH_LABEL2WORD_WORDNET)
    for obj in tqdm(sentences_with_amr_container, leave = False):
        try:
                    ...
                    if node1_label != node2_label and (node1, node2) not in graph_edges and (node2, node1) not in graph_edges:
                        ...
                        if (node1_label, node2_label) in cached_edges:
                            ...
                            for cached_edge in cached_edges[(node1_label, node2_label)]:
                                graph.add_edge(node1, node2, label = cached_edge[0], example = cached_edge[1])
                            ...
                            for new_edge in get_relations_between_words(node1_label, node2_label):
                                graph.add_edge(node1, node2, label = new_edge['edge'][1], example = new_edge['example'])
                                cached_edges[(node1_label, node2_label)].append((new_edge['edge'][1], new_edge['example']))
            ...
            for edge_type in graph_stats.keys():
                added_edges = [(label2word[edge[0]], label2word[edge[1]]) for edge in graph.edges(data = True) if edge[2]['label'] == edge_type]
                if len(added_edges):
                    logger.info(
    ...
    for edge_type in statistics.keys():
        logger.info(f"Number of edges added based on words being {edge_type}: {np.sum(statistics[edge_type]) / len(sentences_with_amr_container)}")
        logger.info(10 * '##')
