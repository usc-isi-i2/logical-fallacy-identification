...
def do_train_process(config=None):
    with wandb.init(config=config):
        ...
        train_df['label'] = label_encoder.transform(
            train_df['label'])
        dev_df['label'] = label_encoder.transform(
            dev_df['label'])
        ...
        model = AutoModelForSequenceClassification.from_pretrained(
            "ynie/electra-large-discriminator-snli_mnli_fever_anli_R1_R2_R3-nli", num_labels=len(list(label_encoder.classes_)), classifier_dropout=config.classifier_dropout, ignore_mismatched_sizes=True)
        ...
        training_args = TrainingArguments(
            do_eval=True,
            do_train=True,
            output_dir='./sentence_segmentation_classification',
...
if __name__ == '__main__':
    ...
    parameters_dict = {
        ...
        'data_dir': {
            'values': ['data/logic_data/finegrained/']
        },
        'batch_size': {
            'values': [16]
        },
    ...
    sweep_id = wandb.sweep(
        sweep_config, project='sentence segment classification')
